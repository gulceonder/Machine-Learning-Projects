...
eff_vac=ctrl.Antecedent(np.arange(-1.0, 1.1, 0.1), 'Effective Vaccination')
...
cur_vac['low'] = fuzz.trapmf(cur_vac.universe, [0, 0, 0.2, 0.6])
cur_vac['medium'] = fuzz.trimf(cur_vac.universe, [0.3, 0.6, 0.9])
cur_vac['high'] = fuzz.trapmf(cur_vac.universe, [0.6, 0.9, 1.0, 1.0])

eff_vac['low'] = fuzz.trapmf(eff_vac.universe, [-1., -1., -0.5, 0.])
eff_vac['medium'] = fuzz.trimf(eff_vac.universe, [-0.5, 0., 0.5])
eff_vac['high'] = fuzz.trapmf(eff_vac.universe, [0., 0.5, 1.0, 1.0])
...
#set rules
rule1 = ctrl.Rule(cur_vac['low'] & eff_vac['low'] , out_rate['high'])
rule2 = ctrl.Rule(cur_vac['low'] & eff_vac['medium'] , out_rate['high'])
rule3 = ctrl.Rule(cur_vac['low'] & eff_vac['high'] , out_rate['medium'])

rule4 = ctrl.Rule(cur_vac['medium']& eff_vac['low'], out_rate['high'])
rule5 = ctrl.Rule(cur_vac['medium']& eff_vac['high'], out_rate['low'])
rule6 = ctrl.Rule(cur_vac['medium']& eff_vac['medium'], out_rate['medium'])

rule7 = ctrl.Rule(cur_vac['high']& eff_vac['low'] , out_rate['medium'])
rule8 = ctrl.Rule(cur_vac['high']& eff_vac['high'] , out_rate['low'])
rule9 = ctrl.Rule(cur_vac['high']& eff_vac['medium'] , out_rate['low'])

#rule1.view()

#create a control system to simulate
vaccination_ctrl = ctrl.ControlSystem([rule1, rule2, rule3,rule4,rule5,rule6,rule7,rule8,rule9])
...
for i in range(200):
    ...
    vaccinate.input['Effective Vaccination'] = pi_dot
...
vac_cost=vaccination.vaccination_rate_curve_[0:30]
cost=np.sum(vac_cost)*100


vaccination.viewVaccination(30, cost, save_dir='/Users/gulceonder/Desktop/webpage/webdevelopment', filename='vsc', show_plot=True)
#plot effective vacc rate curve
