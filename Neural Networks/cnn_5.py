...
class cnn_5(torch.nn.Module):
    def __init__(self):
        ...
        self.conv2= nn.Conv2d(16, 8, 3,1,'same')
        self.conv3= nn.Conv2d(8, 8, 3,1,'same')
        self.conv4= nn.Conv2d(8, 16, 3,1,'same')
        self.conv5= nn.Conv2d(16, 16, 3,1,'same')

        self.fc1 = torch.nn.Linear(16*7*7, 10)
        self.relu = torch.nn.ReLU()
    def forward(self, x):
        x = F.relu(self.conv1(x)) #x shape : [64, 16, 28, 28]
        x = F.relu(self.conv2(x))# x shape: [64, 8, 28, 28]
        x = F.relu(self.conv3(x))# x shape: [64, 8, 28, 28]
        x = self.pool(F.relu(self.conv3(x))) #x shape: [64, 8, 14, 14]
        x = F.relu(self.conv4(x))  #x shape: [64, 8, 14, 14]
        x = self.pool(F.relu(self.conv5(x))) #x shape: [64, 8, 7, 7]
