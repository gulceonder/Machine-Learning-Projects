...
class cnn_3(torch.nn.Module):
    def __init__(self):
        ...
        self.conv2= nn.Conv2d(16, 8, 7,1,'same')
        self.conv3= nn.Conv2d(8, 16, 5,1,'same')

        self.fc1 = torch.nn.Linear(16*7*7, 10)
        self.relu = torch.nn.ReLU()
    def forward(self, x):
        x = F.relu(self.conv1(x)) #x shape : [64, 16, 28, 28]
        x = self.pool(F.relu(self.conv2(x)))# xx shape: [64, 8, 14, 14]
        x = self.pool(self.conv3(x)) #x shape: [64, 8, 7, 7]
...
weights_data =np.zeros((10,64,784))#weights data for layer 1
...
def train( model, loss_fn, optimizer,epoch,run):
    ...
    for batch, (X, y) in enumerate(train_dataloader):
        ...
        if batch % 10 == 0:
            print(model.conv1.weight.data.numpy().shape)
            correct += (pred.argmax(1) == y).type(torch.float).sum().item()
...
def test( model, loss_fn,run):
    ...
    # print(model.cnn1.weight.data.numpy().shape)
